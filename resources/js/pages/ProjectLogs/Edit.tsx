...
export default function EditProjectLog({ projectLog }: Props) {
    const { data, setData, put, processing, errors } = useForm<FormData>({
        name: projectLog.name,
        log_path: projectLog.log_path,
        description: projectLog.description || '',
        is_active: projectLog.is_active,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/project-logs/manage/${projectLog.id}`);
    };

    return (
        <>
            <Head title={`Edit ${projectLog.name}`} />
                ...
                <header className="bg-white shadow-sm border-b">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex justify-between items-center py-6">
                            <div>
                                <h1 className="text-3xl font-bold text-gray-900">Edit Project Log</h1>
                                <p className="mt-1 text-sm text-gray-500">
                                    Perbarui informasi project log
                                </p>
                            </div>
                            <Link
                        ...
                        <div className="px-4 py-5 sm:p-6">
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="flex justify-end space-x-3">
                                    ...
                                    >
                                        {processing ? 'Menyimpan...' : 'Perbarui Project Log'}
                                    </button>
                                </div>
                            </form>
                        </div>
